...
#ifndef __MANIFEST_cnl_H__
#define __MANIFEST_cnl_H__
...
#define MAN_cnl_NUM_MODULES		1

struct adsp_fw_desc {
	struct adsp_fw_header header;
	struct module module[MAN_cnl_NUM_MODULES];
	struct mod_config config[MAN_cnl_NUM_MODULES];
} __attribute__((packed));
...
struct adsp_meta_file_ext {
	...
	struct component_desc comp_desc[MAN_cnl_NUM_MODULES];
} __attribute__((packed));
