...
static int bridge_cb(void *data, struct qemu_io_msg *msg)
{
    ...
    case QEMU_IO_TYPE_REG:
        ...
        adsp_sue_shim_msg(adsp, msg);
        break;
    case QEMU_IO_TYPE_IRQ:
        adsp_sue_irq_msg(adsp, msg);
        break;
    ...
    MachineState *machine, const char *name)
{
    ...
    rom = g_malloc(ADSP_SUE_DSP_ROM_SIZE);
    load_image_size(adsp->rom_filename, rom,
        ADSP_SUE_DSP_ROM_SIZE);
    cpu_physical_memory_write(board->rom.base, rom, ADSP_SUE_DSP_ROM_SIZE);
   ...
   for (i = 0; i < hdr->num_module_entries; i++) {
	...
	mod = &man->desc.module[i];
        ...
        for (j = 0; j < 3; j++) {
            ...
            /* L2 cache */
            if (mod->segment[j].v_base_addr >= ADSP_SUE_DSP_SRAM_BASE &&
                mod->segment[j].v_base_addr < ADSP_SUE_DSP_SRAM_BASE + ADSP_SUE_DSP_SRAM_SIZE) {
	    	soffset = mod->segment[j].v_base_addr - ADSP_SUE_DSP_SRAM_BASE;
            ...
            /* HP SRAM */
            if (mod->segment[j].v_base_addr >= ADSP_SUE_DSP_HP_SRAM_BASE &&
                mod->segment[j].v_base_addr < ADSP_SUE_DSP_HP_SRAM_BASE + ADSP_SUE_DSP_HP_SRAM_SIZE) {
	    	soffset = mod->segment[j].v_base_addr - ADSP_SUE_DSP_HP_SRAM_BASE;
            ...
            /* LP SRAM */
            if (mod->segment[j].v_base_addr >= ADSP_SUE_DSP_LP_SRAM_BASE &&
                mod->segment[j].v_base_addr < ADSP_SUE_DSP_LP_SRAM_BASE + ADSP_SUE_DSP_LP_SRAM_SIZE) {
	    	soffset = mod->segment[j].v_base_addr - ADSP_SUE_DSP_LP_SRAM_BASE;
...
static const struct adsp_desc sue_dsp_desc = {
    ...
    .num_ssp = 3,
    .num_dmac = 2,
    .iram = {.base = ADSP_SUE_DSP_SRAM_BASE, .size = ADSP_SUE_DSP_SRAM_SIZE},
    .dram0 = {.base = ADSP_SUE_DSP_HP_SRAM_BASE, .size = ADSP_SUE_DSP_HP_SRAM_SIZE},
    .lp_sram = {.base = ADSP_SUE_DSP_LP_SRAM_BASE, .size = ADSP_SUE_DSP_LP_SRAM_SIZE},
    .rom = {.base = ADSP_SUE_DSP_ROM_BASE, .size = ADSP_SUE_DSP_ROM_SIZE},

    .mbox_dev = {
        .name = "mbox",
        .reg_count = ARRAY_SIZE(adsp_mbox_map),
        .reg = adsp_mbox_map,
        .desc = {.base = ADSP_SUE_DSP_MAILBOX_BASE, .size = ADSP_SUE_DSP_MAILBOX_SIZE},
    },

    .shim_dev = {
        .name = "shim",
        .reg_count = ARRAY_SIZE(adsp_sue_shim_map),
        .reg = adsp_sue_shim_map,
        .desc = {.base = ADSP_SUE_DSP_SHIM_BASE, .size = ADSP_SUE_SHIM_SIZE},
    },
    ...
    .gp_dmac_dev[0] = {
        ...
        .reg = adsp_gp_dma_map,
        .desc = {.base = ADSP_SUE_DSP_LP_GP_DMA_BASE(0), .size = ADSP_SUE_DSP_LP_GP_DMA_SIZE},
    },
    .gp_dmac_dev[1] = {
        .name = "dmac1",
        .reg_count = ARRAY_SIZE(adsp_gp_dma_map),
        .reg = adsp_gp_dma_map,
        .desc = {.base = ADSP_SUE_DSP_HP_GP_DMA_BASE(0), .size = ADSP_SUE_DSP_HP_GP_DMA_SIZE},
    },
    .ssp_dev[0] = {
        .name = "ssp0",
        .reg_count = ARRAY_SIZE(adsp_ssp_map),
        .reg = adsp_ssp_map,
        .desc = {.base = ADSP_SUE_DSP_SSP_BASE(0), .size = ADSP_SUE_DSP_SSP_SIZE},
    },
    .ssp_dev[1] = {
        .name = "ssp1",
        .reg_count = ARRAY_SIZE(adsp_ssp_map),
        .reg = adsp_ssp_map,
        .desc = {.base = ADSP_SUE_DSP_SSP_BASE(1), .size = ADSP_SUE_DSP_SSP_SIZE},
    },
    .ssp_dev[2] = {
        .name = "ssp2",
        .reg_count = ARRAY_SIZE(adsp_ssp_map),
        .reg = adsp_ssp_map,
        .desc = {.base = ADSP_SUE_DSP_SSP_BASE(2), .size = ADSP_SUE_DSP_SSP_SIZE},
    },
    .io_dev = {
        .name = "io",
        .reg_count = 0,
        .reg = NULL,
        .desc = {.base = ADSP_SUE_DSP_SHIM_BASE, .size = 0x10000},
    },
};
...
static void sue_adsp_init(MachineState *machine)
{
    ...
    adsp->ext_timer = timer_new_ns(QEMU_CLOCK_VIRTUAL, &sue_ext_timer_cb, adsp);
